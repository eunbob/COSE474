...
def image2col(input_data, filter_H, filter_W, stride = 1):
    ...
    imgage = torch.Tensor(input_data)
    col = torch.zeros((B, C, filter_H, filter_W, H_out, W_out))

    for y in range(filter_H):
        y_max = y + stride * H_out
        for x in range(filter_W):
            x_max = x + stride * W_out
            col[:, :, y, x, :, :] = imgage[:, :, y : y_max : stride, x : x_max : stride]

    col = col.permute(0, 4, 5, 1, 2, 3).reshape(B * H_out * W_out, -1)
    return col
...
class nn_convolutional_layer:
    ...
    def forward(self, x):
        ...
        filter_num = self.W.shape[0]
        filter_H = self.W.shape[2]
        filter_W = self.W.shape[3]

        H_out = x.shape[2] - filter_H + 1
        W_out = x.shape[3] - filter_W + 1

        b = self.b.squeeze()

        col = image2col(x, filter_H, filter_W)
        W_col = self.W.reshape(filter_num, -1).T  
        # Convolution 
        out = torch.matmul(col, W_col) + b
...
class nn_max_pooling_layer:
    ...
    def forward(self, x):
        ...
        col = image2col(X, self.pool_size, self.pool_size, self.stride)
        col = col.reshape(-1, self.pool_size * self.pool_size)

        out = torch.amax(col, dim = 1)
        out = out.reshape(B, H_out, W_out, C).permute(0, 3, 1, 2)
