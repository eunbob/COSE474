    ...
    def forward(self, x):
        ...
        for h in range(h_out):
          for w in range(w_out):
            x_crop = x[torch.arange(N), :, h * S : h * S + H2, w * S : w * S + W2]
            x_crop_stretch = x_crop.reshape(N, f_size)
            w_stretch = self.W.reshape(F, f_size)
            out[torch.arange(N), :, h, w] = torch.matmul(x_crop_stretch, w_stretch.T) + self.b.reshape((1, F))
...
class nn_max_pooling_layer:
    ...
    def forward(self, x):
        ...
        w_out = 1 + (W - p_w) // S
        h_out = 1 + (H - p_h) // S
        out = torch.zeros((B,C, w_out, h_out))
            ...
            for c in range(C):
                for w in range(w_out):
                    for h in range(h_out):
                        str_w = w * S
                        str_h = h * S
                        out[b, c, w, h] = torch.max(x[b, c, str_w : str_w + p_w, str_h : str_h + p_h])
