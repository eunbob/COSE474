...
class TF_Encoder_Block(nn.Module):
    def __init__(self, d_model, d_ff, numhead, dropout):    
        ...
        self.layer_norm1 = nn.LayerNorm(d_ff)
        self.layer_norm2 = nn.LayerNorm(d_ff)
    ...
    def forward(self, x, src_batch_lens):
        ...
        multi = self.multihead_attention(x, x, x, src_batch_lens)
        out_1 = self.layer_norm1(x + multi)
        feed_forward = self.feed_forward(out_1)
        out = self.layer_norm2(out_1 + feed_forward)
        return out
...
class TF_Encoder(nn.Module):
    def __init__(self, vocab_size, d_model,
        ...
        self.encoder_blocks = nn.ModuleList([TF_Encoder_Block(d_model, d_ff, numhead, dropout) for _ in range(numlayer)] )
